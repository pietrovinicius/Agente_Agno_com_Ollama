...
- **Injeção de RAG (add_knowledge_to_context):** Corrigido o nome do parâmetro para `add_knowledge_to_context=True` (em substituição ao anterior `add_context`). Isso força a inclusão dos documentos da base de conhecimento diretamente no prompt, resolvendo o problema de respostas vazias.
